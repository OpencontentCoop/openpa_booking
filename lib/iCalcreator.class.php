...
class vcalendar
{
    ...
    {
        $this->prodid = '-//' . $this->unique_id . '//NONSGML kigkonsult.se ' . ICALCREATOR_VERSION . '//' . strtoupper(
                $this->language
            );
    }
    /**
    ...
    {
        $this->unique_id = ( isset( $_SERVER['SERVER_NAME'] ) ) ? gethostbyname(
            $_SERVER['SERVER_NAME']
        ) : 'localhost';
    }
    /*********************************************************************************/
    ...
    function createXprop()
    {
        ...
        foreach ( $this->xprop as $label => $xpropPart )
        {
            ...
            if ( is_array( $xpropPart['value'] ) )
            {
                ...
                {
                    $xpropPart['value'][$pix] = iCalUtilityFunctions::_strrep(
                        $theXpart,
                        $this->format,
                        $this->nl
                    );
            ...
            {
                $xpropPart['value'] = iCalUtilityFunctions::_strrep(
                    $xpropPart['value'],
                    $this->format,
                    $this->nl
                );
        ...
        switch ( $propName )
        {
            ...
            case 'URL':
                ...
                foreach ( $this->components as $cix => $component )
                {
                    ...
                    {
                        $content = ( false === ( $loc = $component->getProperty(
                                'LOCATION'
                            ) ) ) ? '' : $loc . ' ';
                        ...
                        $content .= iCalUtilityFunctions::_geo2str2(
                                $geo['latitude'],
                                iCalUtilityFunctions::$geoLatFmt
                            ) .
                                    iCalUtilityFunctions::_geo2str2(
                                        $geo['longitude'],
                                        iCalUtilityFunctions::$geoLongFmt
                                    ) . '/';
                    }
    ...
    function getConfig( $config = false )
    {
        ...
        switch ( strtoupper( $config ) )
        {
            ...
            case 'DIRECTORY':
                ...
                break;
            case 'DIRFILE':
                return $this->getConfig( 'directory' ) . $this->getConfig(
                    'delimiter'
    ...
    function deleteComponent( $arg1, $arg2 = false )
    {
        ...
        elseif ( ( strlen( $arg1 ) <= strlen( 'vfreebusy' ) )
                 && ( false === strpos(
                    $arg1,
                    '@'
                ) )
        ...
        foreach ( $this->components as $cix => $component )
        {
            ...
            { // array( *[propertyName => propertyValue] )
                ...
                foreach ( $arg1 as $pName => $pValue )
                {
                    ...
                    {
                        $part = ( false !== strpos( $part, ',' ) ) ? explode(
                            ',',
                            $part
                        ) : array( $part );
    ...
    )
    {
        ...
        foreach ( $this->components as $cix => $component )
        {
            ...
            if ( empty( $start ) && ( $component->objName == 'vtodo' )
                 && ( false === ( $start = $component->getProperty(
                        'due',
                        false,
                        true
                    ) ) )
            ...
            $SCbools = array(
                ...
                'dueExist' => false,
                'durationExist' => false,
                'endAllDayEvent' => false
            );
            $recurrid = false;
            $dateFormat = array();
            unset( $end, $startWdate, $endWdate, $rdurWsecs, $rdur, $workstart, $workend ); // clean up
            $startWdate = iCalUtilityFunctions::_SCsetXCurrentDateZ(
                iCalUtilityFunctions::_date2timestamp( $start ),
                $start
            );
            ...
            $endWdate = iCalUtilityFunctions::_SCsetXCurrentDateZ(
                iCalUtilityFunctions::_date2timestamp( $end ),
                $end
            );
            ...
            if ( ( !$any && ( $startWdate >= $startDate ) && ( $startWdate <= $endDate ) )
            ...
            {    // occurs within the period
                ...
                { // split the original component
                    ...
                    { // exclude any recurrence START date, found in exdatelist
                        ...
                        { // iterate
                            if ( isset( $exdatelist[$checkDate] )
                                 ||                   // exclude any recurrence date, found in the exdatelist
                                 ( isset( $recurridList[$checkDate] ) && !$recurrid )
                            )
                            ...
                            iCalUtilityFunctions::_SCsetXCurrentStart(
                                $component,
                                $dateFormat,
                                $checkDate,
                                $rstartYMD,
                            ...
                            iCalUtilityFunctions::_SCsetXCurrentEnd(
                                $component,
                                $dateFormat,
                                $rstart,
                                $rstartYMD,
            ...
            if ( true === $any )
            {
                ...
                $recurlist = array();
                while ( false !== ( $rrule = $component->getProperty(
                        'rrule'
                    ) ) )    // check rrule
                {
                    iCalUtilityFunctions::_recur2date(
                ...
                {  // check rdate
                    ...
                    {
                        if ( is_array( $theRdate ) && ( 2 == count( $theRdate ) )
    ...
    function selectComponents2( $selectOptions )
    {
        ...
        foreach ( $this->components as $cix => $component3 )
        {
            ...
            foreach ( $selectOptions as $propName => $pvalue )
            {
                ...
                elseif ( false === ( $d = $component3->getProperty(
                        $propName
                    ) )
                ) // single occurrence
                {
    ...
    function replaceComponent( $component )
    {
        ...
        if ( ( 'vtimezone' != $component->objName )
             || ( false === ( $tzid = $component->getProperty(
                    'TZID'
                ) ) )
        )
        {
    ...
    function setComponent( $component, $arg1 = false, $arg2 = false )
    {
        ...
        elseif ( in_array(
            strtolower( $arg1 ),
            array( 'vevent', 'vtodo', 'vjournal', 'vfreebusy', 'valarm', 'vtimezone' )
        ) )
        {
            $argType = strtolower( $arg1 );
    ...
    function parse( $unparsedtext = false )
    {
        ...
        if ( isset( $this->unparsed ) && is_array( $this->unparsed )
             && ( 0 < count(
                    $this->unparsed
                ) )
        )
        {
            ...
            foreach ( $proprows as $line )
            {
                ...
                if ( ( 'x-' != strtolower( substr( $propname, 0, 2 ) ) )
                     && !in_array(
                        strtolower( $propname ),
                        $propnames
                    )
        ...
        { // xCal only
            ...
            foreach ( $this->xcaldecl as $declix => $declPart )
            {
                if ( ( 0 < count(
                     ...
                     && isset( $declPart['uri'] )
                     && isset( $declPart['external'] )
                     && isset( $old_xcaldecl['uri'] )
                     && isset( $old_xcaldecl['external'] )
...
class calendarComponent
{
        ...
        if ( is_array( $params ) )
        {
            ...
            foreach ( $params as $optparamlabel => $optparamvalue )
            {
                if ( ( 'X-' != substr(
                     ...
                     && ( ( 'vfreebusy' == $this->objName ) || ( 'valarm' == $this->objName ) )
                )
                {
                    continue;
                }
                switch ( $optparamlabel )
                {
                    ...
                    default:
                        ...
                        {
                            $optparamvalue = substr(
                                $optparamvalue,
                                1,
                                ( strlen( $optparamvalue ) - 2 )
                            );
                        }
    ...
    function createCategories()
    {
        ...
        foreach ( $this->categories as $category )
        {
            ...
            if ( is_array( $category['value'] ) )
            {
                ...
                {
                    $category['value'][$cix] = iCalUtilityFunctions::_strrep(
                        $categoryPart,
                        $this->format,
                        $this->nl
                    );
            ...
            {
                $content = iCalUtilityFunctions::_strrep(
                    $category['value'],
                    $this->format,
                    $this->nl
                );
    ...
    function createComment()
    {
        ...
        foreach ( $this->comment as $commentPart )
        {
            ...
            $attributes = $this->_createParams(
                $commentPart['params'],
                array( 'ALTREP', 'LANGUAGE' )
            );
            $content = iCalUtilityFunctions::_strrep(
                $commentPart['value'],
                $this->format,
    ...
    )
    {
        ...
        $this->completed = iCalUtilityFunctions::_setDate2(
            $year,
            $month,
            $day,
            $hour,
    ...
    )
    {
        ...
        $this->created = iCalUtilityFunctions::_setDate2(
            $year,
            $month,
            $day,
            $hour,
    ...
    )
    {
        ...
        $this->dtend = iCalUtilityFunctions::_setDate(
            $year,
            $month,
            $day,
            $hour,
    ...
    )
    {
        ...
        {
            $this->dtstamp = iCalUtilityFunctions::_setDate2(
                $year,
                $month,
                $day,
                $hour,
    ...
    )
    {
        ...
        $this->dtstart = iCalUtilityFunctions::_setDate(
            $year,
            $month,
            $day,
            $hour,
    ...
    )
    {
        ...
        $this->due = iCalUtilityFunctions::_setDate(
            $year,
            $month,
            $day,
            $hour,
    ...
    )
    {
        ...
        {
            $this->duration = array(
                'value' => iCalUtilityFunctions::_duration2arr(
                    array(
                        'week' => $week,
                        'day' => $day,
                        'hour' => $hour,
    ...
    function setExdate( $exdates, $params = false, $index = false )
    {
        ...
        $input = array(
            'params' => iCalUtilityFunctions::_setParams(
                $params,
                array( 'VALUE' => 'DATE-TIME' )
            )
        );
        $toZ = ( isset( $input['params']['TZID'] )
                 && in_array(
                strtoupper( $input['params']['TZID'] ),
                array( 'GMT', 'UTC', 'Z' )
            ) ) ? true : false;
        ...
        foreach ( $exdates as $eix => $theExdate )
        {
            ...
            {
                if ( isset( $theExdate['tz'] )
                     && !iCalUtilityFunctions::_isOffset(
                        $theExdate['tz']
                    )
                )
                {
    ...
    function setExrule( $exruleset, $params = false, $index = false )
    {
        ...
        iCalUtilityFunctions::_setMval(
            $this->exrule,
            iCalUtilityFunctions::_setRexrule( $exruleset ),
            $params,
            false,
            $index
        );
        ...
        foreach ( $this->freebusy as $freebusyPart )
        {
            if ( empty( $freebusyPart['value'] )
                 || ( ( 1 == count(
                      ...
                      && isset( $freebusyPart['value']['fbtype'] ) )
            )
            {
                if ( $this->getConfig( 'allowEmpty' ) )
                {
    ...
    function setFreebusy( $fbType, $fbValues, $params = false, $index = false )
    {
        ...
        if ( ( !in_array(
                $fbType,
                array( 'FREE', 'BUSY', 'BUSY-UNAVAILABLE', 'BUSY-TENTATIVE' )
            ) )
             && ( 'X-' != substr( $fbType, 0, 2 ) )
        )
        {
        ...
        {   // periods => period
            ...
            { // pairs => singlepart
                ...
                if ( is_array( $fbMember ) )
                {
                    ...
                    { // timestamp value
                        $freebusyPairMember = iCalUtilityFunctions::_timestamp2date(
                            $fbMember['timestamp'],
                            7
                        );
                        $freebusyPairMember['tz'] = 'Z';
    ...
    function createGeo()
    {
        ...
        return $this->_createElement(
            'GEO',
            $this->_createParams( $this->geo['params'] ),
            iCalUtilityFunctions::_geo2str2(
                $this->geo['value']['latitude'],
                iCalUtilityFunctions::$geoLatFmt
            ) .
            ';' . iCalUtilityFunctions::_geo2str2(
                $this->geo['value']['longitude'],
                iCalUtilityFunctions::$geoLongFmt
            )
        );
    }
    ...
    )
    {
        ...
        $this->lastmodified = iCalUtilityFunctions::_setDate2(
            $year,
            $month,
            $day,
            $hour,
    ...
    function createLocation()
    {
        ...
        {
            return ( $this->getConfig( 'allowEmpty' ) ) ? $this->_createElement(
                'LOCATION'
            ) : false;
        }
        $attributes = $this->_createParams(
    ...
    function setOrganizer( $value, $params = false )
    {
        ...
        if ( isset( $this->organizer['params']['SENT-BY'] ) )
        {
            ...
            {
                $this->organizer['params']['SENT-BY'] = 'MAILTO:' . substr(
                        $this->organizer['params']['SENT-BY'],
                        7
                    );
            }
        }
    ...
    function createPercentComplete()
    {
        ...
        if ( !isset( $this->percentcomplete['value'] )
             || ( empty( $this->percentcomplete['value'] )
                  && !is_numeric(
        ...
        foreach ( $rdates as $rpix => $theRdate )
        {
            ...
            foreach ( $theRdate['value'] as $rix => $rdatePart )
            {
                ...
                if ( is_array(
                     ...
                     && isset( $theRdate['params']['VALUE'] )
                     && ( 'PERIOD' == $theRdate['params']['VALUE'] )
                )
                { // PERIOD
                    if ( $utctime )
                    {
                    ...
                    $formatted = iCalUtilityFunctions::_date2strdate(
                        $rdatePart[0]
                    ); // PERIOD part 1
    ...
    function setRdate( $rdates, $params = false, $index = false )
    {
        ...
        $toZ = ( isset( $params['TZID'] )
                 && in_array(
                strtoupper( $params['TZID'] ),
                $zArr
            ) ) ? true : false;
        /*  check if PERIOD, if not set */
        if ( ( !isset( $input['params']['VALUE'] )
               || !in_array(
                    $input['params']['VALUE'],
                    array( 'DATE', 'PERIOD' )
                ) )
             ...
             && isset( $rdates[0][0] )
             && isset( $rdates[0][1] )
             && !isset( $rdates[0]['timestamp'] )
             && ( ( is_array( $rdates[0][0] )
                    && ( isset( $rdates[0][0]['timestamp'] )
                         || iCalUtilityFunctions::_isArrayDate(
                            $rdates[0][0]
                        ) ) )
                  || ( is_string( $rdates[0][0] ) && ( 8 <= strlen( trim( $rdates[0][0] ) ) ) ) )
             && ( is_array( $rdates[0][1] )
                  || ( is_string( $rdates[0][1] )
                       && ( 3 <= strlen(
                            trim( $rdates[0][1] )
                        ) ) ) )
        )
        ...
        foreach ( $rdates as $rpix => $theRdate )
        {
            ...
            if ( is_array( $theRdate ) )
            {
                    ...
                    foreach ( $theRdate as $rix => $rPeriod )
                    {
                        ...
                        if ( is_array( $rPeriod ) )
                        {
                            ...
                            {    // timestamp
                                ...
                                $inputab = iCalUtilityFunctions::_timestamp2date(
                                    $rPeriod,
                                    $parno
                                );
                            }
                            ...
                            {
                                $d = ( 3 < count( $rPeriod ) ) ? iCalUtilityFunctions::_chkDateArr(
                                    $rPeriod,
                                    $parno
                                ) : iCalUtilityFunctions::_chkDateArr( $rPeriod, 6 );
                                if ( isset( $d['tz'] ) && ( 'Z' != $d['tz'] )
                                     && iCalUtilityFunctions::_isOffset(
                                        $d['tz']
                                    )
                            ...
                            { // text-date
                                $inputab = iCalUtilityFunctions::_strdate2date(
                                    reset( $rPeriod ),
                                    $parno
                                );
                                unset( $inputab['unparsedtext'] );
                ...
                {                                                                  // date[-time]
                    ...
                    if ( isset( $inputa['tz'] ) && ( 'Z' != $inputa['tz'] )
                         && iCalUtilityFunctions::_isOffset(
                            $inputa['tz']
                        )
                    )
    ...
    )
    {
        ...
        $this->recurrenceid = iCalUtilityFunctions::_setDate(
            $year,
            $month,
            $day,
            $hour,
    ...
    function createRelatedTo()
    {
        ...
        foreach ( $this->relatedto as $relation )
        {
            ...
            {
                $output .= $this->_createElement(
                    'RELATED-TO',
                    $this->_createParams( $relation['params'] ),
    ...
    function createResources()
    {
        ...
        foreach ( $this->resources as $resource )
        {
            ...
            if ( is_array( $resource['value'] ) )
            {
                ...
                {
                    $resource['value'][$rix] = iCalUtilityFunctions::_strrep(
                        $resourcePart,
                        $this->format,
                        $this->nl
                    );
    ...
    function setRrule( $rruleset, $params = false, $index = false )
    {
        ...
        iCalUtilityFunctions::_setMval(
            $this->rrule,
            iCalUtilityFunctions::_setRexrule( $rruleset ),
            $params,
            false,
            $index
        );
    ...
    )
    {
        ...
        {  // duration or date in a string
            ...
            { // duration
                $relatedStart = ( isset( $params['RELATED'] )
                                  && ( 'END' == strtoupper(
                            $params['RELATED']
                        ) ) ) ? false : true;
        ...
        )
        {
            ...
            {
                $this->trigger['value'] = iCalUtilityFunctions::_duration2arr(
                    $this->trigger['value']
                );
            }
    ...
    function _createElement( $label, $attributes = null, $content = false )
    {
        ...
        {
            $pos = strrpos( $content, "/" );
            $docname = ( $pos !== false ) ? substr(
                $content,
                ( 1 - strlen( $content ) + $pos )
            ) : $content;
            $this->xcaldecl[] = array(
                'xmldecl' => 'ENTITY'
                ,
        ...
        {
            $content = $this->nl . $this->_createElement(
                    'b64bin',
                    $attachfmttype,
                    $content
                ); // max one attribute
    ...
    function _format_recur( $recurlabel, $recurdata )
    {
        ...
        foreach ( $recurdata as $therule )
        {
            ...
            $attributes = ( isset( $therule['params'] ) ) ? $this->_createParams(
                $therule['params']
            ) : null;
            ...
            foreach ( $therule['value'] as $rulelabel => $rulevalue )
            {
                switch ( strtoupper( $rulelabel ) )
                {
                    ...
                    case 'UNTIL':
                    {
                        ...
                        $content2 .= ';UNTIL=' . iCalUtilityFunctions::_date2strdate(
                                $rulevalue,
                                $parno
                            );
                        break;
        ...
        switch ( $propName )
        {
            ...
            case 'ATTENDEE':
                ...
                while ( is_array(
                        ...
                        && !isset( $this->attendee[$propix] )
                        && ( 0 < count( $this->attendee ) )
                        && ( $propix < end( $ak ) ) )
                {
                    $propix++;
                }
            ...
            case 'CATEGORIES':
                ...
                while ( is_array(
                        ...
                        && !isset( $this->categories[$propix] )
                        && ( 0 < count( $this->categories ) )
                        && ( $propix < end( $ak ) ) )
                {
                    $propix++;
                }
            ...
            case 'COMMENT':
                ...
                while ( is_array(
                        ...
                        && !isset( $this->comment[$propix] )
                        && ( 0 < count( $this->comment ) )
                        && ( $propix < end( $ak ) ) )
                {
                    $propix++;
                }
            ...
            case 'CONTACT':
                ...
                while ( is_array(
                        ...
                        && !isset( $this->contact[$propix] )
                        && ( 0 < count( $this->contact ) )
                        && ( $propix < end( $ak ) ) )
                {
                    $propix++;
                }
            ...
            case 'CREATED':
                ...
                break;
            case 'DESCRIPTION':
                $ak = ( is_array( $this->description ) ) ? array_keys(
                    $this->description
                ) : array();
                while ( is_array(
                            $this->description
                        )
                        && !isset( $this->description[$propix] )
                        && ( 0 < count( $this->description ) )
                        && ( $propix < end( $ak ) ) )
                {
                    $propix++;
                }
            ...
            case 'DURATION':
                ...
                $value = ( $specform && isset( $this->dtstart['value'] ) && isset( $this->duration['value'] ) ) ? iCalUtilityFunctions::_duration2date(
                    $this->dtstart['value'],
                    $this->duration['value']
                ) : $this->duration['value'];

                return ( $inclParam ) ? array(
                    'value' => $value,
            ...
            case 'FREEBUSY':
                ...
                while ( is_array(
                        ...
                        && !isset( $this->freebusy[$propix] )
                        && ( 0 < count( $this->freebusy ) )
                        && ( $propix < end( $ak ) ) )
                {
                    $propix++;
                }
            ...
            case 'RDATE':
                ...
                while ( is_array( $this->rdate ) && !isset( $this->rdate[$propix] )
                        && ( 0 < count(
                            $this->rdate
                        ) )
                        && ( $propix < end( $ak ) ) )
            ...
            case 'RELATED-TO':
                ...
                while ( is_array(
                        ...
                        && !isset( $this->relatedto[$propix] )
                        && ( 0 < count( $this->relatedto ) )
                        && ( $propix < end( $ak ) ) )
                {
                    $propix++;
                }
            ...
            case 'REPEAT':
                ...
                break;
            case 'REQUEST-STATUS':
                $ak = ( is_array( $this->requeststatus ) ) ? array_keys(
                    $this->requeststatus
                ) : array();
                while ( is_array(
                            $this->requeststatus
                        )
                        && !isset( $this->requeststatus[$propix] )
                        && ( 0 < count( $this->requeststatus ) )
                        && ( $propix < end( $ak ) ) )
                {
                    $propix++;
                }
            ...
            case 'RESOURCES':
                ...
                while ( is_array(
                        ...
                        && !isset( $this->resources[$propix] )
                        && ( 0 < count( $this->resources ) )
                        && ( $propix < end( $ak ) ) )
                {
                    $propix++;
                }
            ...
            case 'RRULE':
                ...
                while ( is_array( $this->rrule ) && !isset( $this->rrule[$propix] )
                        && ( 0 < count(
                            $this->rrule
                        ) )
                        && ( $propix < end( $ak ) ) )
    ...
    function _getProperties( $propName, & $output )
    {
        ...
        if ( !in_array(
            strtoupper( $propName ),
            array( 'ATTENDEE', 'CATEGORIES', 'CONTACT', 'RELATED-TO', 'RESOURCES' )
        )
        )
        {
            return $output;
            ...
            switch ( strtoupper( $propname ) )
            {
                case 'ATTENDEE':
                    foreach ( $propAttr as $pix => $attr )
                    {
                        if ( !in_array(
                            strtoupper( $pix ),
                            array( 'MEMBER', 'DELEGATED-TO', 'DELEGATED-FROM' )
                        )
                        )
                        {
                            continue;
                ...
                case 'SUMMARY':
                    ...
                    $this->setProperty(
                        $propname,
                        iCalUtilityFunctions::_strunrep( $line ),
                        $propAttr
                    );
                    break;
                case 'REQUEST-STATUS':
                    $values = explode( ';', $line, 3 );
                    $values[1] = ( !isset( $values[1] ) ) ? null : iCalUtilityFunctions::_strunrep(
                        $values[1]
                    );
                    $values[2] = ( !isset( $values[2] ) ) ? null : iCalUtilityFunctions::_strunrep(
                        $values[2]
                    );
                    $this->setProperty(
                        $propname
...
class iCalUtilityFunctions
{
    ...
    public static function _chkDateArr( $datetime, $parno = false )
    {
        ...
        if ( 3 != $parno )
        {
            ...
            if ( isset( $output['tz'] ) && ( ( '+0000' == $output['tz'] ) || ( '-0000' == $output['tz'] ) || ( '+000000' == $output['tz'] ) || ( '-000000' == $output['tz'] ) ) )
            {
                $output['tz'] = 'Z';
            }
        }
    ...
    public static function _chkdatecfg( $theDate, & $parno, & $params )
    {
        ...
        else
        {
            ...
            if ( is_array( $theDate ) )
            {
                ...
                {
                    $tzid = ( isset( $theDate['tz'] ) ) ? $theDate['tz'] : ( 7 == count(
                            $theDate
                        ) ) ? end( $theDate ) : null;
                }
                if ( !empty( $tzid ) )
    ...
    public static function convEolChar( & $text, $nl )
    {
        ...
        $text = str_replace(
            array( "\r\n", "\n\r", "\n", "\r" ),
            iCalUtilityFunctions::$baseDelim,
            $text
        );
        /* fix empty lines */
        $text = str_replace(
            iCalUtilityFunctions::$baseDelim . iCalUtilityFunctions::$baseDelim,
            iCalUtilityFunctions::$baseDelim . str_pad( '', 75 ) . iCalUtilityFunctions::$baseDelim,
            $text
        );
        /* fix line folding */
    ...
    )
    {
        ...
        if ( !empty( $from ) )
        {
            ...
            $dateFrom->modify(
                '-7 month'
            );                           // set $dateFrom to seven month before the lowest date
        }
        else
        {
            $from = reset(
        ...
        else
        {
            ...
            $dateTo->modify(
                '+7 month'
            );                             // set $dateTo to seven month after the highest date
        ...
        {                  // all transitions in date-time order!!
            ...
            {                           // standard timezone
                if ( !empty( $stdIx ) && isset( $transTemp[$stdIx]['offsetfrom'] )
                     && // check for any repeating rdate's (in order)
        ...
        {      // if no match found
            ...
            {            // then we use the last transition (before startdate) for the tz info
                ...
                $date->modify(
                    $prevTrans['offsetfrom'] . 'seconds'
                );     // convert utc date to local date
                $d = $date->format(
                    'Y-n-j-G-i-s'
                );                     // set date to array to ease up dtstart setting
                $d = explode( '-', $d );
                $prevTrans['time'] = array(
                    'year' => $d[0],
                    'month' => $d[1],
                    'day' => $d[2],
                    'hour' => $d[3],
            ...
            {                        // or we use the timezone identifier to BUILD the standard tz info (?)
                ...
                $transTemp[0] = array(
                    'time' => $date->format( 'Y-m-d\TH:i:s O' )
                    ,
        ...
        { // create standard/daylight subcomponents
            ...
            if ( isset( $trans['offsetfrom'] ) )
            {
                $scomp->setProperty(
                    'tzoffsetfrom',
                    iCalUtilityFunctions::offsetSec2His( $trans['offsetfrom'] )
                );
            }
            $scomp->setProperty(
                'tzoffsetto',
                iCalUtilityFunctions::offsetSec2His( $trans['offset'] )
            );
    ...
    {
        if ( isset( $duration['week'] ) || isset( $duration['day'] ) || isset( $duration['hour'] ) || isset( $duration['min'] ) || isset( $duration['sec'] ) )
        {
            $ok = true;
        }
        else
        ...
        if ( ( isset( $duration['hour'] ) && ( 0 < $duration['hour'] ) ) || ( isset( $duration['min'] ) && ( 0 < $duration['min'] ) ) || ( isset( $duration['sec'] ) && ( 0 < $duration['sec'] ) ) )
        {
            $output .= 'T';
    ...
    public static function _isArrayDate( $input )
    {
        ...
        $input = iCalUtilityFunctions::_strdate2date(
    ...
    public static function _isOffset( $input )
    {
        ...
        elseif ( ( 7 <= strlen( $input ) )
                ...
                array( '+', '-' )
                 ...
                 && ( '000000' <= substr( $input, -6 ) )
                 && ( '999999' >= substr( $input, -6 ) )
        )
        {
            return true;
        }
    ...
    public static function ms2phpTZ( & $timezone )
    {
        ...
        $searchText = trim(
            str_replace( array( '(', ')', '&', ',', '  ' ), ' ', substr( $search, ( $pos + 1 ) ) )
        );
    ...
    public static function _recur2date( & $result, $recur, $wdate, $startdate, $enddate = false )
    {
        ...
        { // save start date + weekno
            ...
            if ( 'YEARLY' == $recur['FREQ'] )
            {
                ...
                $wdatets = iCalUtilityFunctions::_date2timestamp( $wdate );
                iCalUtilityFunctions::_stepdate(
                    $enddate,
                    $endDatets,
                    array( 'year' => 1 )
        ...
        while ( true )
        {
            ...
            if ( $year_old != $wdate['year'] )
            {
                ...
                { // count up and update up-counters
                    ...
                    for ( $d = 1; $d <= $mcnt; $d++ )
                    {
                        ...
                        {
                            $daycnts[$m][$d]['weekno_up'] = (int)date(
                                'W',
            ...
            if ( 1 < $recur['INTERVAL'] )
            {
                ...
                $intervalix = iCalUtilityFunctions::_recurIntervalIx(
                    $recur['FREQ'],
                    $wdate,
                    $wkst
                );
            ...
            {
                $updateOK = iCalUtilityFunctions::_recurBYcntcheck(
                    $recur['BYMONTH']
                    ,
                    $wdate['month']
                    ,
            ...
            if ( $updateOK && isset( $recur['BYDAY'] ) )
            {
                ...
                { // single day, opt with year/month day order no
                    ...
                    if ( isset( $recur['BYDAY'][0] ) )
                    {
                        ...
                        {
                            $daynosw = iCalUtilityFunctions::_recurBYcntcheck(
                                $recur['BYDAY'][0]
                                ,
            ...
            if ( $updateOK )
            {
                if ( isset( $recur['BYSETPOS'] )
                     && ( in_array(
                        $recur['FREQ'],
                        array( 'YEARLY', 'MONTHLY', 'WEEKLY', 'DAILY' )
                    ) )
                )
                {
                    ...
                    else
                    {
                        if ( ( isset( $recur['FREQ'] ) && ( 'YEARLY' == $recur['FREQ'] ) && ( $bysetposYold == $wdate['year'] ) )
                             || ( isset( $recur['FREQ'] ) && ( 'MONTHLY' == $recur['FREQ'] ) && ( ( $bysetposYold == $wdate['year'] ) && ( $bysetposMold == $wdate['month'] ) ) )
                             || ( isset( $recur['FREQ'] ) && ( 'DAILY' == $recur['FREQ'] ) && ( ( $bysetposYold == $wdate['year'] ) && ( $bysetposMold == $wdate['month'] ) && ( $bysetposDold == $wdate['day'] ) ) )
                        )
                        {
            ...
            if ( $updateOK && isset( $recur['BYSETPOS'] ) )
            {
                ...
                elseif ( isset( $recur['FREQ'] ) && ( 'WEEKLY' == $recur['FREQ'] ) )
                {
                    $weekno = (int)date(
                        'W',
                        mktime( 0, 0, $wkst, $wdate['month'], $wdate['day'], $wdate['year'] )
                    );
     ...
     * @return void
     */
    static function _SCsetXCurrentEnd(
        $comp,
        $dateFormat,
        $timestamp1,
        $cmpYMD1,
     ...
     * @return void
     */
    static function _SCsetXCurrentStart(
        $comp,
        $dateFormat,
        $timestamp1,
        $cpmYMD1 = false,
    ...
    public static function _setRexrule( $rexrule )
    {
        ...
        foreach ( $rexrule as $rexrulelabel => $rexrulevalue )
        {
            ...
            else
            {
                ...
                if ( iCalUtilityFunctions::_isArrayTimestampDate(
                    $rexrulevalue
                )
                ) // timestamp, always date-time UTC
                {
                    $input[$rexrulelabel] = iCalUtilityFunctions::_timestamp2date(
                        $rexrulevalue,
                        7,
                        'UTC'
                    );
    ...
    )
    {
        ...
        if ( iCalUtilityFunctions::_isArrayDate( $year ) )
        {
            ...
            elseif ( !isset( $input['value']['tz'] ) && isset( $month['TZID'] )
                     && iCalUtilityFunctions::_isOffset(
                    $month['TZID']
                )
            )
            ...
            $input['params'] = iCalUtilityFunctions::_setParams(
                $month,
                array( 'VALUE' => 'DATE-TIME' )
            );
            ...
            if ( ( 6 <= $parno ) && isset( $input['value']['tz'] ) && ( 'Z' != $input['value']['tz'] )
                 && iCalUtilityFunctions::_isOffset(
                    $input['value']['tz']
                )
            )
        ...
        elseif ( isset( $input['params']['TZID'] ) )
        {
            if ( ( 'UTC' == strtoupper( $input['params']['TZID'] ) )
                 || ( 'GMT' == strtoupper(
                        $input['params']['TZID']
                    ) )
            )
        ...
        {
            if ( ( 'UTC' == strtoupper( $input['value']['tz'] ) )
                 || ( 'GMT' == strtoupper(
                        $input['value']['tz']
                    ) )
            )
    ...
    )
    {
        ...
        { // ex. 2006-08-03 10:12:18
            ...
            if ( ( !isset( $input['value']['tz'] ) || empty( $input['value']['tz'] ) ) && isset( $input['params']['TZID'] )
                 && iCalUtilityFunctions::_isOffset(
                    $input['params']['TZID']
                )
            )
        ...
        else
        {
            ...
            if ( ( isset( $tz ) && iCalUtilityFunctions::_isOffset( $tz ) )
                 || ( isset( $input['params']['TZID'] )
                      && iCalUtilityFunctions::_isOffset(
                        $input['params']['TZID']
                    ) )
            )
            {
                if ( !isset( $tz ) && isset( $input['params']['TZID'] )
                     && iCalUtilityFunctions::_isOffset(
                        $input['params']['TZID']
                    )
                )
     ...
     * @return void
     */
    public static function _setMval(
        & $valArr,
        $value,
        $params = false,
        $defaults = false,
    ...
    )
    {
        ...
        $valArr[$index] = array(
            'value' => $value,
            'params' => iCalUtilityFunctions::_setParams( $params, $defaults )
        );
        ksort( $valArr );
    }
    ...
    public static function _setSortArgs( & $c, $sortArg = false )
    {
        ...
        {
            $c->srtk[1] = iCalUtilityFunctions::_strdate2date(
                $d[1]
        ...
        if ( false === ( $c->srtk[2] = $c->getProperty(
                'created'
            ) )
    ...
    static $parValPrefix = array(
        'MStz' => array( 'utc-', 'utc+', 'gmt-', 'gmt+' )
        ,
    ...
    public static function _splitContent( & $line, & $propAttr = null )
    {
        ...
        {
            if ( !$WithinQuotes && ( ':' == $line[$cix] ) && ( substr( $line, $cix, 3 ) != '://' )
                 && ( !in_array(
                    strtolower( substr( $line, $cix - 6, 4 ) ),
                    iCalUtilityFunctions::$parValPrefix['MStz']
                ) )
                 && ( !in_array(
                    strtolower( substr( $line, $cix - 3, 4 ) ),
                    iCalUtilityFunctions::$parValPrefix['Proto3']
                ) )
                 && ( !in_array(
                    strtolower( substr( $line, $cix - 4, 5 ) ),
                    iCalUtilityFunctions::$parValPrefix['Proto4']
                ) )
                 && ( !in_array(
                    strtolower( substr( $line, $cix - 6, 7 ) ),
                    iCalUtilityFunctions::$parValPrefix['Proto6']
                ) )
            )
            {
                $attrEnd = true;
    ...
    public static function _strDate2arr( & $input )
    {
        ...
        if ( ( ( 0 > $temp['hour'] ) || ( $temp['hour'] > 23 ) ) || ( ( 0 > $temp['min'] ) || ( $temp['min'] > 59 ) ) )
        {
            return false;
        }
    ...
    public static function _strdate2date( $datetime, $parno = false, $wtz = null )
    {
        ...
        elseif ( empty( $wtz ) && ctype_digit( substr( $datetime, 0, 4 ) )
                 && ctype_digit(
                substr( $datetime, -2, 2 )
            )
                 && iCalUtilityFunctions::_strDate2arr( $datetime )
        )
        {
            $output = $datetime;
            if ( !empty( $tz ) )
        ...
        switch ( $format )
        {
            ...
            default:
                ...
                while ( isset( $string[$pos] ) )
                {
                    ...
                    {
                        $string = substr( $string, 0, $pos ) . "\\" . substr(
                                $string,
                                ( $pos + 1 )
                            );
                        $pos += 1;
     ...
     * @return bool
     */
    public static function transformDateTime(
        & $date,
        $tzFrom,
        $tzTo = 'UTC',
        $format = 'Ymd\THis'
    )
    {
        ...
        else
        {
            if ( iCalUtilityFunctions::_isArrayDate( $date ) )
            {
                ...
                $date = iCalUtilityFunctions::_date2strdate(
                    iCalUtilityFunctions::_chkDateArr( $date )
                );
            }
            if ( 'Z' == substr( $date, -1 ) )
            {
    ...
    while ( false !== ( $component = $calendar->getComponent() ) )
    {
            ...
            switch ( strtolower( $prop ) )
            {
                ...
                case 'recurrence-id':
                    if ( false !== ( $content = $component->getProperty( $prop, false, true ) ) )
                    {
                        $type = ( isset( $content['params']['VALUE'] ) && ( 'DATE' == $content['params']['VALUE'] ) ) ? 'date' : 'date-time';
                        unset( $content['params']['VALUE'] );
                        if ( ( isset( $content['params']['TZID'] ) && empty( $content['params']['TZID'] ) )
                             || @is_null(
                                $content['params']['TZID']
                            )
                        )
        ...
        while ( false !== ( $subcomp = $component->getComponent() ) )
        {
            ...
            foreach ( $subCompProps as $prop )
            {
                ...
                {
                    case 'attach':          // may occur multiple times, below
                        while ( false !== ( $content = $subcomp->getProperty(
                                $prop,
                                false,
                                true
                            ) ) )
...
function _addXMLchild( & $parent, $name, $type, $content, $params = array() )
{
    ...
    switch ( $type )
    {
        ...
        case 'duration':
            ...
            $v = $child->addChild(
                $type,
                $output . iCalUtilityFunctions::_duration2str( $content )
            );
            break;
        case 'geo':
            if ( !empty( $content ) )
        ...
        case 'period':
            ...
            foreach ( $content as $period )
            {
                ...
                else
                {
                    $v2 = $v1->addChild(
                        'duration',
                        iCalUtilityFunctions::_duration2str( $period[1] )
                    );
                }
            }
            break;
        ...
        case 'rstatus':
            $v = $child->addChild(
                'code',
                number_format( (float)$content['statcode'], 2, '.', '' )
            );
...
function XMLgetProps( $iCal, $xml )
{
    while ( !empty( $xml ) )
    {
            ...
            while ( !empty( $xml3 ) )
            {
                ...
                if ( in_array( $paramKey, array( 'DELEGATED-FROM', 'DELEGATED-TO', 'MEMBER' ) ) )
                {
                    while ( !empty( $xml4 ) )
                    {
                        ...
                        {
                            $params[$paramKey][] = XMLgetTagContent1(
                                $xml4,
                                'cal-address',
                                $endIx4
                            );
                ...
                else
                {
                    if ( !isset( $params[$paramKey] ) )
                    {
                        $params[$paramKey] = html_entity_decode(
                            XMLgetTagContent2( $xml4, $pType, $endIx4 )
                        );
                    }
                    else
                    {
        ...
        $valueType = false;
        $value = ( !empty( $xml2 ) || ( '0' == $xml2 ) ) ? XMLgetTagContent2(
            $xml2,
            $valueType,
            $endIx3
        ) : '';
        switch ( $propName )
        {
            ...
            case 'RESOURCES':
                ...
                while ( !empty( $xml2 ) )
                {
                    $tValue[] = html_entity_decode(
                        XMLgetTagContent2( $xml2, $valueType, $endIx4 )
                    );
            ...
            case 'GEO':
                ...
                $tValue['longitude'] = XMLgetTagContent1(
                    substr( $xml2, $endIx3 ),
                    'longitude',
                    $endIx3
                );
...
function XMLgetTagContent1( $xml, $tagName, & $endIx = 0 )
{
    ...
    while ( false !== substr( $xml, $sx1, 1 ) )
    {
        if ( ( false !== substr( $xml, ( $sx1 + $strlen + 1 ), 1 ) )
             && ( strtolower( "<$tagName>" ) == strtolower(
                    substr( $xml, $sx1, ( $strlen + 2 ) )
                ) )
        )
        {
            break;
...
function getTzOffsetForDate( $timezonesarray, $tzid, $timestamp )
{
    ...
    if ( is_array( $tzdatearray ) )
    {
        ...
        elseif ( $timestamp >= $tzdatearray[count( $tzdatearray ) - 1]['timestamp'] )
        {
            ...
            $tzoffset['offsetHis'] = $tzdatearray[count(
                                                      $tzdatearray
                                                  ) - 1]['tzafter']['offsetHis'];
            $tzoffset['offsetSec'] = $tzdatearray[count(
                                                      $tzdatearray
                                                  ) - 1]['tzafter']['offsetSec'];
...
function expandTimezoneDates( $vtzc )
{
    ...
    if ( ( '-' != substr( (string)$tzbefore['offsetSec'], 0, 1 ) )
         && ( '+' != substr(
                (string)$tzbefore['offsetSec'],
                0,
                1
            ) )
    )
    ...
    if ( ( '-' != substr( (string)$tzafter['offsetSec'], 0, 1 ) )
         && ( '+' != substr(
                (string)$tzafter['offsetSec'],
                0,
                1
            ) )
    )
    ...
    if ( is_array( $rrule ) )
    {
        if ( $rrule['FREQ'] == 'YEARLY' )
        {
            ...
            while ( $offsetchangetimestamp < $stoptimestamp && ( $stopcount == 0 || $count < $stopcount ) )
            {
                ...
                $offsetchangetimestamp = strtotime(
                    "+" . ( ( isset( $rrule['INTERVAL'] ) && ( $rrule['INTERVAL'] != 0 ) ) ? $rrule['INTERVAL'] : 1 ) . " year",
                    $offsetchangetimestamp
                );
                $count += 1;
            }
